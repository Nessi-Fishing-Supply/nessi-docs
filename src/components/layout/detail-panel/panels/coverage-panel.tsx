  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '6px', marginBottom: '12px' }}>
        <span style={{ fontSize: '10px', padding: '2px 8px', borderRadius: '10px', background: `${personaCfg.color}1a`, color: personaCfg.color }}>
          {personaCfg.label}
        </span>
      </div>
      ...
      <div style={{ marginBottom: '12px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', marginBottom: '4px' }}>
          ...
          <span style={{ color: STATUS_CONFIG.built.color }}>{builtPct}% ({built}/{total})</span>
        </div>
        <div style={{ height: '4px', background: 'var(--bg-raised)', borderRadius: '2px', overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${builtPct}%`, background: STATUS_CONFIG.built.color, borderRadius: '2px' }} />
        </div>
      </div>
      ...
      <div style={{ marginBottom: '16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', marginBottom: '4px' }}>
          ...
          <span style={{ color: STATUS_CONFIG.tested.color }}>{testedPct}% ({tested}/{total})</span>
        </div>
        <div style={{ height: '4px', background: 'var(--bg-raised)', borderRadius: '2px', overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${testedPct}%`, background: STATUS_CONFIG.tested.color, borderRadius: '2px' }} />
        </div>
      </div>
      ...
      {planned.length > 0 && (
        <>
          ...
          {planned.map((s) => (
            <div key={s.id} style={{ fontSize: '10px', color: '#b84040', padding: '3px 8px', background: 'rgba(184,64,64,0.06)', borderRadius: '3px', marginBottom: '2px' }}>
              {s.label}
            </div>
          ))}
        </>
      ...
      {builtUntested.length > 0 && (
        <>
          ...
          {builtUntested.map((s) => (
            <div key={s.id} style={{ fontSize: '10px', color: '#e27739', padding: '3px 8px', background: 'rgba(226,119,57,0.06)', borderRadius: '3px', marginBottom: '2px' }}>
              {s.label}
            </div>
          ))}
        </>
